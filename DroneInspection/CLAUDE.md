...
| `placeObjects` / `qteTargets` | 配置。点検ポイント（QTE の対象）9か所を 30〜280m にほぼ等間隔で置き、6種類を最低1回ずつ出す。避けるだけの障害物 18個は QTE の地点の前後とチェックポイントの近くには置かない |
| `startQte` / `qtePress` / `resolveQte` / `updateQte` | 点検 QTE（ピント合わせ）。点検ポイントの 36 手前でスロー（時間の進み 8%）になり、縮む輪がピントの枠に重なった瞬間に押す。PERFECT（±0.08秒）/ GOOD（±0.22秒）で成功、MISS は壁の異常なら見逃し・障害物ならぶつかって -15 |
...
- ランクは成功率（成功数 / 9）で決める。S 85% / A 65% / B 40%（ゲーム内の目安）
- 開発機での通しプレイは約45秒（QTE が合計約14秒、QTE の間の飛行は平均約3秒）
...
- デルタタイム対応（経過秒で動く）
- 難易度・ランクのバランス／点検と回避の矛盾：点検を QTE にし、オブジェクトを 80個 → 点検ポイント9か所＋避けるだけの障害物18個に減らして解消。点検に成功した障害物は砕けて消える
...
3. **難易度の調整**：遠心力の強さ（`animate` 内の係数 0.08）、増水の水位、QTE の判定の幅（`QTE_PERFECT` / `QTE_GOOD`）とランクのしきい値は、実際に遊んで調整が必要
